...
/**
 * Callback hủy / từ chối nhận đơn về EMS.
 *
 * @param array $extra reason (khuyến nghị), note
 */
function sendCancelCallback(int $order_id, array $extra = []): array
{
    global $conn;
    global $callbackConfig;

    $stmt = $conn->prepare('SELECT id, ems_code FROM emslss_orders WHERE id=? LIMIT 1');
    $stmt->bind_param('i', $order_id);
    $stmt->execute();
    $order = $stmt->get_result()->fetch_assoc();
    if (!$order) {
        return ['success' => false, 'http_code' => 0, 'response' => 'Order not found'];
    }
    ...
    if ($reason === '') {
        $metaStmt = $conn->prepare("
            ...
            WHERE order_id=? AND meta_key='lss_reject_reason'
    ...
    $url = $callbackConfig['pickup_url'] ?? $callbackConfig['delivery_url'] ?? '';
    ...
    if ($url === '') {
        return ['success' => false, 'http_code' => 0, 'response' => 'Missing callback URL config'];
    }

    $bodyToSend = $payload;
    if ($payloadMode === 'dto_wrapper') {
        $bodyToSend = json_encode(['dto' => $data, 'time' => $eventTime], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
    }
    if ($useExecuteFormat) {
    ...
    apiLog('CALLBACK_CANCEL', $bodyToSend, json_encode([
        'success' => $success,
        'http_code' => $http_code,
        'body' => $response,
    ...
    $trackNote = $success
        ? 'EMS cancel callback success'
        : 'EMS cancel callback fail: HTTP ' . $http_code;
    $tr = $conn->prepare('INSERT INTO emslss_tracking(order_id, status, note, created_by) VALUES(?,?,?,NULL)');
